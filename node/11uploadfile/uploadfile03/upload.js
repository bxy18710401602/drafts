...
function uploadFile (val) {
  ...
  if (fileSize < tenMillionBytes) { // 如果文件小于10M，就直接发送了，否则就使用分片上传
    ...
    let requestObj = {
      ...
      data: formData,
      success: (data) => {
        displayUploadedImage(data); // 将上传完的图片展示在页面中
      },
      fail: (data) => {
        console.log(data); 
      }
    ...
    for (let i = 0; i < blockLength; i++) {
      ...
      console.log(Math.floor(blockConent.size / 1024 * 1024), '选中的文件裁剪后');
      let reader = new FileReader();
      reader.readAsArrayBuffer(blockConent);
      reader.addEventListener('load', (event) => {
        let arrayBufferFile = event.target.result;
        // let int8ArrayFile= new Int8Array(arrayBufferFile);
        console.log(arrayBufferFile, '看看当前拿到的内容是啥')
        ...
        formData.set('blockContent', arrayBufferFile);
        // 请求配置
        let config = {
          'Content-Type': 'multipart/form-data',
